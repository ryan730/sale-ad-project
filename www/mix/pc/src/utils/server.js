...
export default async (url = '', data = {}, type = 'GET', contentType = 'form') => {
    ...
    let requestConfig = {
        ...
        // `onUploadProgress` 允许为上传处理进度事件,注意这个事件会触发options预检
    ...
    console.log('request:', arguments, requestConfig);
    return new Promise((resolve, reject) => {
        NProgress.start();
        var server = axios(requestConfig);
        server.then(response => {
            ...
            if (code === '0') {
                ...
                window.customGlobal.globalVM.$message({
                    type: 'error',
