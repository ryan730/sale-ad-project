...
<script>
    ...
    import {getMyOutStock} from '@/api/api';
    import Vue from 'vue';
    ...
    export default {
        ...
        created(){
            this.apiGetMyOutStock();
        },
        methods:{
            async apiGetMyOutStock(){
                let userID = this.$store.getters.userID;
                this.tableData=[];//置空之前数据
                try {
                    //employeeID:客户状态(默认有效客户)
                    //pageNum:页码（默认第一页）
                    //pageSize:每页显示条数（默认10条）
                    const res = await getMyOutStock({employeeID:userID,pageNum:'1',pageSize:'10'});
                    console.log('getMyInStock-result:',res.result);
