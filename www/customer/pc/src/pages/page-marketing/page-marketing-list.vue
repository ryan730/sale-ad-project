...
<script>
    ...
    import {getSalerOrder,getMyUserFollow} from '@/api/api';
    import Vue from 'vue';
    ...
    export default {
        ...
        created(){
            //this.apiGetSalerOrder();
            this.apiGetMyUserFollow();
        },
        methods:{
            async apiGetMyUserFollow(){
                let userID = this.$store.getters.userID;
                try {
                    const res = await getMyUserFollow({employeeID:userID});
                    console.log('MyUserFollow-result:',res.result);
            ...
            async apiGetSalerOrder(){
                this.tableData=[];//置空之前数据
                try {
                    //salerID:员工ID（销售员）
                    //customer:客户ID
                    //placeOrderType:下单类型 1:自动下单；2：代客下单
                    //pageNum:页码（默认第一页）
                    //pageSize:每页显示条数（默认10条）
                    const res = await getSalerOrder({salerID:'1',customer:'1',placeOrderType:'1',pageNum:'1',pageSize:'10'});
                    console.log('getSalerOrder-result:',res.result);
